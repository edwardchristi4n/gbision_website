...
export default function ManagePrograms() {
  ...
  const fetchItems = () => {
    setLoading(true)
    // community=general → hanya program umum gereja (bukan program komunitas)
        ...
        <div className="mb-8 p-6 bg-white border border-slate-200 rounded-xl shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Jadwal (Contoh: Setiap Minggu, 10.00)</label>
                <input 
                  type="text"
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:border-navy"
            ...
            <div>
              ...
              {formData.image_url ? (
                <div className="relative w-full h-48 rounded-xl overflow-hidden border border-slate-200">
                  ...
                  <button type="button" onClick={() => setFormData({...formData, image_url: ""})} className="absolute top-2 right-2 bg-white text-red-500 p-1.5 rounded-lg shadow hover:bg-red-50">
                    <Trash2 size={16} />
                  </button>
                </div>
              ) : (
            ...
            <div className="flex items-center gap-2 py-2">
              <input type="checkbox" id="isActive" checked={formData.is_active} onChange={e => setFormData({...formData, is_active: e.target.checked})} className="w-4 h-4 text-orange" />
              <label htmlFor="isActive" className="text-sm font-medium text-slate-700 cursor-pointer">Program Aktif (Ditampilkan di Website)</label>
            </div>

            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={() => {setFormOpen(false); setEditingId(null)}} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg font-medium transition">
                Batal
              </button>
            ...
            <div key={item.id} className={`bg-white border ${item.is_active ? 'border-slate-200' : 'border-red-200 opacity-60'} rounded-xl overflow-hidden shadow-sm hover:shadow-md transition`}>
              ...
              <div className="p-4">
                ...
                <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                    <button onClick={() => handleEdit(item)} className="text-sm font-medium text-navy hover:text-orange flex items-center gap-1 transition">
                        <Edit size={14} /> Edit
                    </button>
                    <button onClick={() => handleDelete(item.id)} className="text-sm font-medium text-red-500 hover:text-red-600 flex items-center gap-1 transition">
